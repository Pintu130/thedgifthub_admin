...
import { useGetWelcomeContentQuery, useUpdateWelcomeContentMutation } from "@/lib/redux/features/post/postsApiSlice"
import { Button } from "@/components/ui/button"
import Loader from "@/components/loading-screen"
import { useToast } from "@/hooks/use-toast"
import FormInput from "@/components/common/FormInput" // Make sure this path is correct
import AdvancedCKEditor from "@/components/common/advanced-ckeditor"

const WelcomePage = () => {
  const { data, isLoading, isError } = useGetWelcomeContentQuery()
  const [updateWelcomeContent, { isLoading: isSaving }] = useUpdateWelcomeContentMutation()
  ...
  useEffect(() => {
    if (data?.data?.[0]) {
      const welcomeData = data.data[0]
      setWelcomeId(welcomeData._id)
      setEditorData(welcomeData.description || "")
  ...
  const handleSave = async () => {
    ...
    if (editorData.trim() === "") {
      ...
      setEditorError("") // Clear error if valid
    }

    try {
      const res = await updateWelcomeContent({
        id: welcomeId,
        title,
      ...
      toast({
        title: "Welcome content updated",
        description: res?.message || "Welcome content updated successfully.",
      })
    } catch (error: any) {
      toast({
        variant: "destructive",
  ...
  const isFormValid = title.trim() !== "" && editorData.trim() !== "" && !editorError

  return (
    <RootLayout>
      <div className="p-4">
        <h1 className="text-2xl font-bold mb-4 text-customButton-text">Welcome Page Content</h1>

        {(isLoading || isSaving) && <Loader />}

        {isError && (
          <div className="bg-red-100 border border-red-400 text-red-700 p-4 rounded mb-4">
            Error loading welcome content. Please try again.
          </div>
        )}
            ...
            <div className="mt-6">
              ...
              <div className="max-h-[350px] overflow-y-auto border rounded scrollbar-custom">
                <AdvancedCKEditor
                  ...
                  placeholder="Type or paste your welcome content here!"
                />
              </div>
